...
def correlation(target_column, features, df):
    out = (
        ...
        .apply(lambda feature: df.select([feature, target_column]).na.drop("any").corr(feature, target_column))
        .astype(float)
        .fillna(0.0)
    )
    return out
...
def notna(target_column, features, df):
    out = (
        pd.Series(features, index=features)
        .apply(lambda feature: df.select([feature, target_column]).na.drop("any").count())
        .astype(float)
    )
    return out
...
def f_classif(target_column, features, df):
    ...
    avg = (
        groupby.agg({feature: "mean" for feature in features})
        .toPandas()
        .set_index(target_column)
        .rename(lambda colname: colname[4:-1], axis=1)
    )

    var = (
        groupby.agg({feature: "var_pop" for feature in features})
        .toPandas()
        .set_index(target_column)
        .rename(lambda colname: colname[8:-1], axis=1)
    )

    n = (
        groupby.agg({feature: "count" for feature in features})
        .toPandas()
        .set_index(target_column)
        .rename(lambda colname: colname[6:-1], axis=1)
    )
