...
def test_consistency_f_classif():
    ...
    f_classif_bigquery = mrmr.bigquery.f_classif(
        bq_client=bq_client, table_id=df_bigquery, target_column=target_column_classif, features=features
    )
    ...
    assert ((f_classif_pandas - f_classif_spark[f_classif_pandas.index]).abs() < 0.001).all()
    assert ((f_classif_pandas - f_classif_bigquery[f_classif_pandas.index]).abs() < 0.001).all()


def test_consistency_correlation():
    correlation_pandas = mrmr.pandas.correlation(
        target_column=target_column_regression, features=features, X=df_pandas
    )
    correlation_spark = mrmr.spark.correlation(target_column=target_column_regression, features=features, df=df_spark)
    correlation_bigquery = mrmr.bigquery.correlation(
        bq_client=bq_client, table_id=df_bigquery, target_column=target_column_regression, features=features
    )
    ...
    assert ((correlation_pandas - correlation_spark[correlation_pandas.index]).abs() < 0.001).all()
    assert ((correlation_pandas - correlation_bigquery[correlation_pandas.index]).abs() < 0.001).all()


def test_consistency_f_regression():
    f_regression_pandas = mrmr.pandas.f_regression(
        X=df_pandas.loc[:, features], y=df_pandas.loc[:, target_column_regression]
    )
    f_regression_spark = mrmr.spark.f_regression(
        df=df_spark, target_column=target_column_regression, features=features
    )
    f_regression_bigquery = mrmr.bigquery.f_regression(
        bq_client=bq_client, table_id=df_bigquery, target_column=target_column_regression, features=features
    )
    ...
    assert ((f_regression_pandas - f_regression_spark[f_regression_pandas.index]).abs() < 0.001).all()
    assert ((f_regression_pandas - f_regression_bigquery[f_regression_pandas.index]).abs() < 0.001).all()
